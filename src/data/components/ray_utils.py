...
def get_ray_directions(H, W, K, device='cpu', random=False, return_uv=False, flatten=True):
    """
    Get ray directions for all pixels in camera coordinate [right up back].
    old version: Get ray directions for all pixels in camera coordinate [right down front].
    ...
    directions = torch.stack([(u-cx)/fx, (cy-v)/fy, -torch.ones_like(u)], -1)
    if flatten:
